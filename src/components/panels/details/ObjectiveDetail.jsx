...
const tabs = [
  ...
  { id: 'tactics', label: 'Tactics', icon: Layers },
  { id: 'activity', label: 'Activity', icon: Clock },
]

// Child item card
function TacticCard({ item, onNavigate, items }) {
  const parentCount = item.parentIds?.length || 0
  
  return (
    <button
      onClick={() => onNavigate(item)}
      className="w-full flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg hover:border-blue-300 hover:shadow-sm transition-all text-left group"
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <SharedBadge parentCount={parentCount} size="xs" showUsage={false} />
          </div>
...
export default function ObjectiveDetail({
  ...
  // Get child tactics
  const childTactics = (item?.childIds || [])
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-b border-slate-200 bg-slate-50">
        {tabs.map(tab => {
          ...
          const count = tab.id === 'tactics' ? childTactics.length : null
          ...
          <div className="p-4 space-y-6">
            ...
            <div>
              ...
              <div className="bg-slate-50 rounded-lg p-3">
                <p className="text-sm text-slate-600">{item.metric || 'No metrics defined.'}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  Create Region Variant
                </button>
                <button
                  onClick={() => setShowAIGenerate(true)}
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-sm bg-emerald-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition-colors"
                >
            ...
            <div className="pt-4 border-t border-slate-200">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex items-center gap-2 text-slate-500">
                  ...
                  <span>Modified: {new Date(item.lastEditedAt).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-500">
                  <User className="w-4 h-4" />
                  <span>By: {item.lastEditedBy || 'Unknown'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 space-y-4">
            ...
            {childTactics.length > 0 ? (
              ...
              <div className="text-center py-8 text-slate-500">
                ...
                <p className="text-sm">No tactics yet</p>
                <p className="text-xs mt-1">Add tactics to break down this objective</p>
              </div>
            )}
          </div>
        )}
