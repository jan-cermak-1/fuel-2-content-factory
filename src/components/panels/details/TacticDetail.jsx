...
// Child item card
function BestPracticeCard({ item, onNavigate, items }) {
  const parentCount = item.parentIds?.length || 0
  
  return (
    <button
      onClick={() => onNavigate(item)}
      className="w-full flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg hover:border-emerald-300 hover:shadow-sm transition-all text-left group"
    >
...
export default function TacticDetail({
  ...
  // Get children
  const childBestPractices = (item?.childIds || [])
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-b border-slate-200 bg-slate-50">
        {tabs.map(tab => {
          ...
          const count = tab.id === 'bestPractices' ? childBestPractices.length : null
          ...
          return (
            ...
            >
              ...
              {tab.label}
              {count !== null && (
                <span className={`px-1.5 py-0.5 text-xs rounded-full ${isActive ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-600'}`}>
                  {count}
                </span>
              )}
            </button>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-700">
                Best Practices ({childBestPractices.length})
              </h3>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => onAddChild?.()}
                ...
                <button
                  onClick={() => onUseTemplate?.('bestPractice')}
            ...
            {childBestPractices.length > 0 ? (
              <div className="space-y-2">
                {childBestPractices.map(bp => (
                  <BestPracticeCard 
                    key={bp.id} 
                    item={bp} 
                    items={items}
              ...
              <div className="text-center py-8 text-slate-500">
                ...
                <p className="text-sm">No best practices yet</p>
              </div>
            )}
          </div>
        )}
